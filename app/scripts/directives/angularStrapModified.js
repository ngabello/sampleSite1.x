...
angular.module('mgcrea.ngStrap.datepicker', [
  ...
  .provider('$datepicker', function() {
    ...
    this.$get = ["$window", "$document", "$rootScope", "$sce", "$dateFormatter", "datepickerViews", "$tooltip1", "$timeout", function($window, $document, $rootScope, $sce, $dateFormatter, datepickerViews, $tooltip1, $timeout) {
      ...
      function DatepickerFactory(element, controller, config) {
        ...
        var $datepicker = $tooltip1(element, angular.extend({}, defaults, config));
...
angular.module('mgcrea.ngStrap.dropdown', ['mgcrea.ngStrap.tooltip'])
  ...
  .provider('$dropdown', function() {
    ...
    this.$get = ["$window", "$rootScope", "$tooltip1", "$timeout", function($window, $rootScope, $tooltip1, $timeout) {
      ...
      function DropdownFactory(element, config) {
        ...
        $dropdown = $tooltip1(element, options);
...
angular.module('mgcrea.ngStrap.popover', ['mgcrea.ngStrap.tooltip'])
  ...
  .provider('$popover', function() {
    ...
    this.$get = ["$tooltip1", function($tooltip1) {
      ...
      function PopoverFactory(element, config) {
        ...
        var $popover = $tooltip1(element, options);
...
angular.module('mgcrea.ngStrap.select', ['mgcrea.ngStrap.tooltip', 'mgcrea.ngStrap.helpers.parseOptions'])
  ...
  .provider('$select', function() {
    ...
    this.$get = ["$window", "$document", "$rootScope", "$tooltip1", "$timeout", function($window, $document, $rootScope, $tooltip1, $timeout) {
      ...
      function SelectFactory(element, controller, config) {
        ...
        $select = $tooltip1(element, options);
...
angular.module('mgcrea.ngStrap.tooltip', ['mgcrea.ngStrap.helpers.dimensions'])
  ...
  .provider('$tooltip1', function() {
    ...
    this.$get = ["$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$sce", "dimensions", "$$rAF", "$timeout", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $sce, dimensions, $$rAF, $timeout) {
      ...
      function TooltipFactory(element, config) {
        ...
        var options = $tooltip1.$options = angular.extend({}, defaults, config);
        $tooltip1.$promise = fetchTemplate(options.template);
        var scope = $tooltip1.$scope = options.scope && options.scope.$new() || $rootScope.$new();
        ...
        $tooltip1.$isShown = scope.$isShown = false;
        ...
        if(options.contentTemplate) {
          $tooltip1.$promise = $tooltip1.$promise.then(function(template) {
        ...
        $tooltip1.$promise.then(function(template) {
        ...
        $tooltip1.init = function() {
          ...
          if(options.show) {
            scope.$$postDigest(function() {
              options.trigger === 'focus' ? element[0].focus() : $tooltip1.show();
            });
          }

        };

        $tooltip1.destroy = function() {
        ...
        $tooltip1.enter = function() {

          clearTimeout(timeout);
          hoverState = 'in';
          if (!options.delay || !options.delay.show) {
            return $tooltip1.show();
          }

          timeout = setTimeout(function() {
            if (hoverState ==='in') $tooltip1.show();
          }, options.delay.show);

        };

        $tooltip1.show = function() {
          if (!options.bsEnabled || $tooltip1.$isShown) return;

          scope.$emit(options.prefixEvent + '.show.before', $tooltip1);
          var parent, after;
          ...
          tipScope = $tooltip1.$scope.$new();
          tipElement = $tooltip1.$element = tipLinker(tipScope, function(clonedElement, scope) {});
          ...
          $tooltip1.$isShown = scope.$isShown = true;
          safeDigest(scope);
          $$rAF(function () {
            $tooltip1.$applyPlacement();
        ...
        function enterAnimateCallback() {
          scope.$emit(options.prefixEvent + '.show', $tooltip1);
        }

        $tooltip1.leave = function() {

          clearTimeout(timeout);
          hoverState = 'out';
          if (!options.delay || !options.delay.hide) {
            return $tooltip1.hide();
          }
          timeout = setTimeout(function () {
            if (hoverState === 'out') {
              $tooltip1.hide();
        ...
        var _blur;
        $tooltip1.hide = function(blur) {

          if(!$tooltip1.$isShown) return;
          scope.$emit(options.prefixEvent + '.hide.before', $tooltip1);
        ...
        function leaveAnimateCallback() {
          scope.$emit(options.prefixEvent + '.hide', $tooltip1);
        ...
        $tooltip1.toggle = function() {
          $tooltip1.$isShown ? $tooltip1.leave() : $tooltip1.enter();
        };

        $tooltip1.focus = function() {
          tipElement[0].focus();
        };

        $tooltip1.setEnabled = function(isEnabled) {
          options.bsEnabled = isEnabled;
        };

        // Protected methods

        $tooltip1.$applyPlacement = function() {
        ...
        $tooltip1.$onKeyUp = function(evt) {
          if (evt.which === 27 && $tooltip1.$isShown) {
            $tooltip1.hide();
            evt.stopPropagation();
          }
        };

        $tooltip1.$onFocusKeyUp = function(evt) {
          if (evt.which === 27) {
            element[0].blur();
            evt.stopPropagation();
          }
        ...
        $tooltip1.$onFocusElementMouseDown = function(evt) {
          evt.preventDefault();
          evt.stopPropagation();
          // Some browsers do not auto-focus buttons (eg. Safari)
          $tooltip1.$isShown ? element[0].blur() : element[0].focus();
        };
        ...
        function bindTriggerEvents() {
          ...
          angular.forEach(triggers, function(trigger) {
            if(trigger === 'click') {
              element.on('click', $tooltip1.toggle);
            } else if(trigger !== 'manual') {
              element.on(trigger === 'hover' ? 'mouseenter' : 'focus', $tooltip1.enter);
              element.on(trigger === 'hover' ? 'mouseleave' : 'blur', $tooltip1.leave);
              nodeName === 'button' && trigger !== 'hover' && element.on(isTouch ? 'touchstart' : 'mousedown', $tooltip1.$onFocusElementMouseDown);
            }
          });
        }
        ...
        function unbindTriggerEvents() {
          ...
          for (var i = triggers.length; i--;) {
            ...
            if(trigger === 'click') {
              element.off('click', $tooltip1.toggle);
            } else if(trigger !== 'manual') {
              element.off(trigger === 'hover' ? 'mouseenter' : 'focus', $tooltip1.enter);
              element.off(trigger === 'hover' ? 'mouseleave' : 'blur', $tooltip1.leave);
              nodeName === 'button' && trigger !== 'hover' && element.off(isTouch ? 'touchstart' : 'mousedown', $tooltip1.$onFocusElementMouseDown);
            }
          }
        }
        ...
        function bindKeyboardEvents() {
          if(options.trigger !== 'focus') {
            tipElement.on('keyup', $tooltip1.$onKeyUp);
          } else {
            element.on('keyup', $tooltip1.$onFocusKeyUp);
          }
        }

        function unbindKeyboardEvents() {
          if(options.trigger !== 'focus') {
            tipElement.off('keyup', $tooltip1.$onKeyUp);
          } else {
            element.off('keyup', $tooltip1.$onFocusKeyUp);
          }
        }
        ...
        function destroyTipElement() {
          ...
          if($tooltip1.$isShown && tipElement !== null) {
            if(options.autoClose) {
              unbindAutoCloseEvents();
            }
          ...
          if(tipElement) {
            tipElement.remove();
            tipElement = $tooltip1.$element = null;
          }
        }
  ...
  .directive('bsTooltip', ["$window", "$location", "$sce", "$tooltip1", "$$rAF", function($window, $location, $sce, $tooltip1, $$rAF) {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, transclusion) {
        ...
        // Initialize popover
        var tooltip = $tooltip1(element, options);
...
angular.module('mgcrea.ngStrap.typeahead', ['mgcrea.ngStrap.tooltip', 'mgcrea.ngStrap.helpers.parseOptions'])
  ...
  .provider('$typeahead', function() {
    ...
    var defaults = this.defaults = {
      ...
      placement: 'bottom-left',
      template: 'scripts/directives/views/angular-strap-modified.tpl.html',
      trigger: 'focus',
      container: false,
      keyboard: true,
      html: false,
    ...
    this.$get = ["$window", "$rootScope", "$tooltip1", "$timeout", function($window, $rootScope, $tooltip1, $timeout) {
      ...
      function TypeaheadFactory(element, controller, config) {
        ...
        $typeahead = $tooltip1(element, options);
        var parentScope = config.scope;
        ...
        $typeahead.$onKeyDown = function(evt) {
          ...
          //  }else {
          //    scope.driverForm.driverOccupation.$setValidity('required', false);
          //    controller.$setViewValue(null);
          //  }
          //}
          if(evt.keyCode === 13 && scope.$matches.length){
              $typeahead.select(scope.$activeIndex);
              controller.$setValidity('list', true);
            }

          if(evt.keyCode === 13 && !/^[0-9]+$/.test(controller.$modelValue)){
            controller.$setViewValue(null);
            scope.driverForm.driverOccupation.$setValidity('list', false);
          }
          // Navigate with keyboard
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        scope.$watch(attr.ngModel, function(newValue, oldValue) {
          ...
          if(new RegExp('^[0-9]+$').test(newValue)) scope.numberValue = newValue;
        ...
        controller.$render = function () {
          // console.warn('$render', element.attr('ng-model'), 'controller.$modelValue', typeof controller.$modelValue, controller.$modelValue, 'controller.$viewValue', typeof controller.$viewValue, controller.$viewValue);
          if(controller.$isEmpty(controller.$viewValue) && scope.$modelValue > 0){
            if(controller.$dirty || scope.$modelValue >= 400){
              element.val('');
              controller.$setViewValue(null);
              //controller.$render();
            }
            else {
              var viewDescription = _.findWhere(scope.occupations, {Value: scope.$modelValue});
              element.val(viewDescription.Description);
              controller.$setViewValue(scope.$modelValue);
              //controller.$render();
              controller.$setValidity('list', true);
              return controller.$setValidity('required', true);
            }
          }
        ...
        //Fix for firefox. explicitly set model value
        element.blur(function(){
          var viewDescription = _.findWhere(scope.occupations, {Description: scope.$modelValue}) || _.findWhere(scope.occupations, {Value: scope.$modelValue});
          if(!viewDescription) {
            controller.$setViewValue(null);
            return scope.driverForm.driverOccupation.$setValidity('list', false);
          }else {
            element.val(viewDescription.Description);
            controller.$setViewValue(scope.numberValue);
            return scope.driverForm.driverOccupation.$setValidity('list', true);
          }
        });
...
angular.module('mgcrea.ngStrap.timepicker', [
  ...
  .provider('$timepicker', function() {
    ...
    this.$get = ["$window", "$document", "$rootScope", "$sce", "$dateFormatter", "$tooltip1", "$timeout", function($window, $document, $rootScope, $sce, $dateFormatter, $tooltip1, $timeout) {
      ...
      function timepickerFactory(element, controller, config) {
        ...
        var $timepicker = $tooltip1(element, angular.extend({}, defaults, config));
        var parentScope = config.scope;
